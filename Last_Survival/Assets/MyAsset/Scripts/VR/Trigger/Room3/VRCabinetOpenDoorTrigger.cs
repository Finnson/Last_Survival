...
public class VRCabinetOpenDoorTrigger : MonoBehaviour
{
        ...
        if (isTrigger)
        {
            if (vrinput.rightTriggerAxis.x > 0.5f && Vector3.Angle(vrinput.right.transform.forward, lockDir) < 80.0f)
            {
                ...
                {//turn on right
                    ...
                    {
                        isEnd = true;
                        gameController.Interact("Cabinet_3");//open the door
                        clearStatus();
                    }
    ...
    {
        if (packageController.curItem == "CabinetKey_2" && packageController.prevItem == "CabinetKey_2" && vrinput.rightTriggerAxis.x > 0.5f && !isEnd)
        {
            if (Vector3.Angle(vrinput.right.transform.forward, lockDir) < 80.0f)
            {//forward must point to the lock
                isTrigger = true;
